...
EXISTING_INSTANCE_IDS=`aws ec2 describe-instances --filters ${FILTERS} --output text --query Reservations[*].Instances[*].[InstanceId]`
...
IP_ADDRESS=`aws ec2 describe-instances --instance-ids ${instanceId} --output text --query Reservations[*].Instances[*].[PrivateIpAddress]`
...
RECORD_SET=`aws route53 list-resource-record-sets --hosted-zone-id Z3R2PID5NAV5C --output text --query "ResourceRecordSets[?Name=='${DOMAIN}.qa.sli.io.']"`
